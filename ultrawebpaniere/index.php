		...
		<script>
			...
			function updateProduitsStack(produits, stack) {
				...
				stack.forEach((element) => {
					...
					button4.onclick = () => {
						ajax("panier.php?a=add&id=" + element.id, (responseText) => {
							updatePanier();
						});
					};
			...
			function updatePanier() {
				...
				ajax("panier.php?a=list", (responseText) => {
					var ids = JSON.parse(responseText);
					if (ids.length == 0) {
						updatePanierStack(panier, []);
					} else {
						var stack = [];
			...
			function updatePanierStack(panier, stack) {
				...
				stack.forEach((element) => {
					...
					input2.step = 1;
					input2.onchange = () => {
						ajax("panier.php?a=set&id=" + element.id + "&value=" + input2.value, (responseText) => {
							updatePanier();
						});
					};
					...
					button4.onclick = () => {
						ajax("panier.php?a=rem&id=" + element.id, (responseText) => {
							updatePanier();
						});
					};
					cell4.appendChild(button4);
