...
<html>
	...
	<body>
		...
		<script>
			...
			function updatePanierStack(stack) {
				...
				stack.forEach((element) => {
					...
					input2.onchange = () => {
						...
						ajax_json("api/panier.php?a=set&id=" + element.id + "&value=" + input2.value, (response) => {
					...
					button3.onclick = () => {
						panier.style.backgroundColor = "#AAA";
						ajax_json("api/panier.php?a=rem&id=" + element.id, (response) => {
							if (response.status == "error") { console.log(response.description); }
							updatePanierIds(response.panier);
						});
					};
			...
			panier.style.backgroundColor = "#AAA";
			ajax_json("api/panier.php", (response) => {
				if (response.status == "error") { console.log(response.description); }
				updatePanierIds(response.panier);
			});
		</script>
