...
if ($auteur = $request1->fetch(PDO::FETCH_ASSOC)) {
	$request2 = $pdo->prepare("SELECT * FROM blagues WHERE id_auteur = :id");
	$request2->bindValue(":id", $auteur['id_auteur'], PDO::PARAM_INT);
    $request2->execute();
...
$response["data"] = $request2->fetchAll(PDO::FETCH_ASSOC);
echo json_encode($response);
die();
